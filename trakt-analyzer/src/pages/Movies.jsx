...
function MovieCard({ item, index, onClick }) {
  ...
  const title = enriched.title_cn || movie?.title || '未知';
        ...
        <div className="p-3.5 bg-[#1C1C1E]">
          ...
          {enriched.directors && enriched.directors.length > 0 && (
            <p className="text-[10px] text-white/30 mt-1.5 truncate">
              导演: {enriched.directors.slice(0, 2).join(', ')}
            </p>
          )}
        </div>
      </div>
...
function MovieDetail({ item, onClose }) {
  const movie = item.movie;
  const enriched = item._enriched || {};
  const posterUrl = enriched.poster;
      ...
      <div className="max-w-2xl w-full bg-[#1C1C1E] backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden animate-scale-in border border-white/10" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6">
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            ...
            {enriched.runtime && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 rounded-xl text-sm text-white/70">
                <span>⏱️</span> {enriched.runtime} 分钟
              </div>
            )}
            {enriched.vote_average && (
            ...
            {enriched.douban_rating && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 rounded-xl text-sm">
                <span>🥬</span>
                <span className="font-semibold text-white">{enriched.douban_rating}</span>
            ...
            {item._watchCount > 1 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-[#FF9500]/20 rounded-xl text-sm text-[#FF9500]">
                <span>🔄</span> 看过 {item._watchCount} 次
              </div>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            {enriched.directors && enriched.directors.length > 0 && (
              <div className="bg-white/10 rounded-2xl p-3">
                <h4 className="text-xs font-semibold text-white mb-1">导演</h4>
                <p className="text-sm text-white/50">{enriched.directors.join(', ')}</p>
              </div>
            )}
          ...
          <div className="pt-4 border-t border-white/10 flex items-center justify-between">
            ...
            <div className="flex gap-3">
              {movie?.ids?.imdb && (
                <a href={`https://www.imdb.com/title/${movie.ids.imdb}`} target="_blank" rel="noopener noreferrer" className="text-xs text-[#007AFF] hover:underline">
                  IMDB ↗
                </a>
              )}
              {enriched.douban_url && (
...
export default function Movies() {
  const { movieData, setMovieData, movieLoading, setMovieLoading } = useDataCache();
  ...
  const loadMovies = () => {
    ...
    setError(null);
    setProgress(0);
    setProgressMessage('正在获取电影观看记录...');
    setProgressStep('fetch');
    setLocalMovies([]);
    setEnrichedCount(0);
    setMovieLoading(true);

    controllerRef.current = fetchEnrichedHistory('movies', {
      onProgress: (data) => {
  ...
  const filteredMovies = localMovies.filter(item => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black">
        <div className="text-center animate-fade-in">
          ...
          <button onClick={() => { setMovieData(null); loadMovies(); }} className="px-6 py-3 bg-[#007AFF] text-white rounded-2xl text-sm font-semibold hover:bg-[#007AFF]/90 transition-colors">
            重新加载
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F]">
      ...
      <PosterWall items={localMovies} />

      {/* Decorative background elements - 增强光晕 */}
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute -top-40 -right-40 w-96 h-96 bg-[#007AFF]/15 rounded-full blur-3xl" />
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-[#FF9500]/15 rounded-full blur-3xl" />
        <div className="absolute top-1/3 left-1/4 w-[500px] h-[500px] bg-[#AF52DE]/10 rounded-full blur-3xl" />
        <div className="absolute bottom-1/3 right-1/4 w-[400px] h-[400px] bg-[#34C759]/10 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div className="flex items-center gap-5">
            <div>
              <h1 className="text-3xl font-bold text-white">电影</h1>
              <p className="text-white/40 mt-1">
                共 {localMovies.length} 部
                {enriching && enrichedCount > 0 && (
                  <span className="ml-2 text-[#007AFF]">· 已加载 {enrichedCount}/{localMovies.length} 张海报</span>
                )}
              </p>
            </div>
            {(() => {
              const ratings = localMovies.filter(m => m._enriched?.vote_average);
              ...
              return (
                <div className="relative w-16 h-16 flex-shrink-0">
                  <svg className="w-16 h-16 -rotate-90" viewBox="0 0 36 36">
                    ...
                    <circle cx="18" cy="18" r="15.5" fill="none" stroke="#007AFF" strokeWidth="3"
        ...
        {enriching && (
          <div className="flex items-center gap-2 px-4 py-2.5 bg-white/10 backdrop-blur-sm rounded-2xl text-sm text-[#5AC8FA] font-medium mb-6 animate-fade-in border border-white/10">
            ...
            <span>正在获取海报和详细信息 ({enrichedCount}/{localMovies.length})...</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {sortedMovies.map((item, index) => (
            <MovieCard key={item.movie?.ids?.trakt || index} item={item} index={index} onClick={setSelectedMovie} />
          ))}
        </div>
        ...
        {sortedMovies.length === 0 && (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-semibold text-white mb-2">没有找到匹配的电影</h3>
      ...
      {/* Movie Detail Modal */}
      {selectedMovie && (
        <MovieDetail item={selectedMovie} onClose={() => setSelectedMovie(null)} />
      )}
    </div>
  );
}
