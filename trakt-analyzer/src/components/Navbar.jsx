...
function RefreshTimer({ onRefresh }) {
  ...
  return (
    ...
    >
      ...
      <span className={`text-[11px] font-medium tabular-nums ${isRefreshing ? 'text-[#007AFF]' : 'text-white/40 group-hover:text-white/60'}`}>
          ...
          <div className="flex items-center gap-2">
            ...
            {user && (
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/5 hover:bg-[#FF453A]/15 text-white/40 hover:text-[#FF453A] transition-all group"
                title="退出登录"
              >
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                </svg>
                <span className="text-[12px] font-medium">退出</span>
              </button>
            )}
          </div>
        </div>
